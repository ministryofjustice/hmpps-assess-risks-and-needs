...
class RiskScoresDto(
  ...
  companion object {
    ...
    fun from(allRisksOasysRiskPredictorsDto: AllRisksOasysRiskPredictorsDto?): List<RiskScoresDto> = allRisksOasysRiskPredictorsDto?.assessments?.filter { it.assessmentType in listOf("LAYER3", "LAYER1") }?.map {
      RiskScoresDto(
    ...
    fun fromVersioned(assessment: AllRisksPredictorAssessmentDto): RiskScoresDto = RiskScoresDto(
      groupReconvictionScore = OgrScoreDto(
    ...
    fun fromVersioned(assessment: RisksCrAssPredictorAssessmentDto): RiskScoresDto = RiskScoresDto(
      groupReconvictionScore = OgrScoreDto(
        oneYear = assessment.ogrScoreDto.ogrs31Year,
        twoYears = assessment.ogrScoreDto.ogrs32Year,
        scoreLevel = ScoreLevel.findByType(assessment.ogrScoreDto.ogrs3RiskRecon),
