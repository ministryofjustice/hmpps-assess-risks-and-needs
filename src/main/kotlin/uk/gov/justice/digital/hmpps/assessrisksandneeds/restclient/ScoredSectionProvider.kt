...
object ScoredSectionProvider {
  fun mapSection(needsSection: NeedsSection): (res: ClientResponse) -> Mono<Pair<NeedsSection, ScoredSection>> = { res ->
  ...
  private fun map(needsSection: NeedsSection): (res: ClientResponse) -> Mono<Pair<NeedsSection, ScoredSection>> = when (needsSection) {
  ...
  private fun <T : ScoredSection> Mono<ScoredSectionResponse<T>>.section(needsSection: NeedsSection): Mono<Pair<NeedsSection, ScoredSection>> = mapNotNull { res -> res.section?.let { needsSection to it } }
}
