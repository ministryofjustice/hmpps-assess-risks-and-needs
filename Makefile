...
save-logs: ## Saves docker container logs in a directory defined by OUTPUT_LOGS_DIR=
	...
	docker logs ${PROJECT_NAME}-oasys-api-1 > ${OUTPUT_LOGS_DIR}/oasys-api.log
...
db-port-forward-pod: ## Creates a DB port-forwarding pod in your currently active Kubernetes context
	...
	INSTANCE_ADDRESS=$$(kubectl get secret hmpps-assess-risks-and-needs-rds-instance -o json | jq -r '.data.rds_instance_address' | base64 --decode) \
...
db-connection-string: ## Outputs a DB connection string that will let you connect to the remote DB through the port-forwarding pod. Override the local port with DB_PORT_FORWARD_PORT=XXXX
	@DATABASE_USERNAME=$$(kubectl get secret hmpps-assess-risks-and-needs-rds-instance -o json | jq -r '.data.database_username' | base64 --decode) \
	DATABASE_PASSWORD=$$(kubectl get secret hmpps-assess-risks-and-needs-rds-instance -o json | jq -r '.data.database_password' | base64 --decode) \
	DATABASE_NAME=$$(kubectl get secret hmpps-assess-risks-and-needs-rds-instance -o json | jq -r '.data.database_name' | base64 --decode) \
